...
void FPSActor::actorInput(const InputState& inputState)
{
	...
	if( inputState.keyboard.getKeyValue(SDL_SCANCODE_X))
	{
		verticalSpeed += 400.0f;
	}
	if (inputState.keyboard.getKeyValue(SDL_SCANCODE_C))
	{
		verticalSpeed -= 400.0f;
	}
	moveComponent->setVerticalSpeed(verticalSpeed);
...
//Yaw Rotation
	...
	float pitchMouseSpeed = 0.0f;
	if (y != 0)
	{
		pitchMouseSpeed = y / maxMouseSpeed;
		pitchMouseSpeed *= maxPitchSpeed;
	}
	cameraComponent->setPitchSpeed(pitchMouseSpeed);
